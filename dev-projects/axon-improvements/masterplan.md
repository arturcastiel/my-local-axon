...
|---|---|---|---|
| **dag-consistency/** | ✓ 1-gate MERGED (MR !6) | `R_DAG_CONSISTENT` wired as a fail-closed BLOCK crucible control (`dag-consistent`) — universal DAG.json + synapse-graph drift gate, verified clean (146 edges/507 neurons). NEXT: **2-cascade** (wire the 7 mutation programs — divide/combine/pr-create/pr-link/phase-new/plan-master — to call `dag.py` ops so structure changes cascade into DAG.json; substantial) → 3-nest (phase-graph DAG.json + neuron `dag:` obligation) | — |
